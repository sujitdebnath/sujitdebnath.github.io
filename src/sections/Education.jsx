    ...
    <section id="education" className="px-6 py-10 sm:py-12">
      <div className="mx-auto max-w-content">
        ...
        <ol className="relative">
          <div
            aria-hidden="true"
            className="absolute left-[7.5rem] top-2 hidden h-[calc(100%-1rem)] w-px bg-line dark:bg-seam sm:block"
          />
          {education.map((ed, i) => (
            <Reveal
              as="li"
              key={ed.degree}
              delay={i * 0.06}
              className="relative grid grid-cols-1 gap-2 py-6 sm:grid-cols-[7rem_1fr] sm:gap-8"
            >
              <div className="flex items-center gap-3 sm:justify-end sm:text-right">
                <span
                  ...
                  className={`hidden h-2 w-2 shrink-0 rounded-full sm:block ${
                    ed.current
                      ? "bg-marker"
                      : "bg-ink-faint dark:bg-parchment-faint"
                  }`}
                />
              ...
              <div>
                ...
                <p className="text-xs text-ink-faint dark:text-parchment-faint">
                  {ed.location}
                </p>
                {ed.details && (
